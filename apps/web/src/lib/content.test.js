...
test("loads the B2 curriculum tree for the teacher workspace", async () => {
  ...
  assert.equal(workspace.summary.lessonCount, 24);
  ...
  assert.equal(workspace.summary.plannedResourceCount, 3);
  ...
  assert.equal(b2.chapters[2].id, "SH-HS-MATH-HJ-B2-C09");
  assert.equal(b2.chapters[2].sections[1].lessons[0].id, "SH-HS-MATH-HJ-B2-C09-L03");
  assert.equal(b2.chapters[2].sections[3].lessons[1].id, "SH-HS-MATH-HJ-B2-C09-L07");
});
