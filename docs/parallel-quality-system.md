...
- 总控线程只做方向、质量闸门、分支核验、集成、推送和调度；修复、生产、审核、新功能实现必须拆给独立 `codex/*` 工作线程，不能在总控线程直接生产资源或实现功能。
...
- 跨资源公式显示问题可能被误当作“小样式问题”。凡用户可见区域仍出现 `theta`、`lambda`、`omega`、`kpi`、`pi/2`、横向纯文本 slash 分式或英文变量占位，必须按数学显示缺陷处理，不能建议进入更高质量状态。
...
- 任一资源存在未修复的数学显示问题，例如希腊字母未符号化、`kπ` 写成 `kpi`、分式不是上下结构、公式与教材书写明显不一致，暂停状态升级，先开独立修复线。
- 总控上下文开始变长或任务涉及具体资源修复、生产、审核、新功能实现时，总控只生成提示词和质量闸门，不直接在总控线程实现。
