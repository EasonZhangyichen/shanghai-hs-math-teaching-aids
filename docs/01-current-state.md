...
- 已创建首版资源工厂工作流：
  ...
  - `content/production/resource-backlog.json` 当前列出必修第二册第 7 章、第 8 章和第 9 章 draft 候选共 28 个资源工作单元：25 个已实现、3 个仍为 `planned`；第 7 章 15 个均已实现，第 8 章 10 个数字化必要性筛选后的候选均已落地，第 9 章 3 个 Applet 候选仅为课程图谱 planned item。当前 Applet 为 17 个总计、14 个已实现、3 个待创建；Manim 为 4 个总计、4 个已实现、0 个待创建；Diagnosis 为 7 个总计、7 个已实现、0 个待创建；每个 item 都包含推荐分支、下一步动作和可复制到新 Codex 对话的 `threadPrompt`。8.3 暂缓资源和第 9 章未终核候选均不进入 scaffold 生产队列。
...
- 第 7 章的前置章节节点仍是引用占位；第 9 章“复数”已新增 draft 图谱和数字化必要性初筛，但尚未完成纸质教材/已登录 dolearning 终核。
...
- 已建立首版资源生产 backlog，可按单个资源工作单元开启新对话，避免全高中内容挤在一个上下文中；当前必修第二册第 7 章 15 个资源工作单元和第 8 章 10 个筛选后候选均已落地，第 9 章新增 3 个 draft planned Applet 候选，并已补充 Applet / Manim / Diagnosis 全类型 scaffold 命令以减少后续手工建目录成本。
- 当前第 7 章和第 8 章 backlog 已无 `planned` item；第 9 章新增 3 个 planned Applet 候选仅用于记录数字化必要性，不应在教材目录终核前 scaffold。下一步应对已生成的第 8 章 Applet / Manim / Diagnosis 逐个进入单资源精修，或优先推进第 7 章资源的数学审校、课堂节奏试读和状态升级，而不是继续新增同质资源。
