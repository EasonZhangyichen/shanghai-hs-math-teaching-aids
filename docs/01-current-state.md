...
- 已创建首版资源工厂工作流：
  ...
  - `content/production/resource-backlog.json` 当前列出必修第二册第 7 章和第 8 章共 33 个资源工作单元：第 7 章 15 个均已实现，第 8 章 18 个为 `planned`；Applet 为 18 个总计、8 个已实现、10 个待创建；Manim 为 6 个总计、3 个已实现、3 个待创建；Diagnosis 为 9 个总计、4 个已实现、5 个待创建；每个 item 都包含推荐分支、下一步动作和可复制到新 Codex 对话的 `threadPrompt`。第 8 章 planned item 在 scaffold 前需要先通过数字化必要性筛选。
...
- 必修第二册第 8 章“平面向量”已完成首版结构化整理、dolearning 来源核对记录和 planned resource 数字化必要性筛选落地，但尚未完成纸质教材/已登录 dolearning 终核；尤其需要确认 8.3 是教材四小节拆分，还是 dolearning 三课时分组。第 8 章尚未创建对应资源包；当前 production backlog 只应反映 10 个必要性明确或已收窄的候选资源，8.3 暂缓资源不进入 scaffold 生产队列。
...
- 已建立首版资源生产 backlog，可按单个资源工作单元开启新对话，避免全高中内容挤在一个上下文中；当前必修第二册第 7 章 15 个资源工作单元已全部落地，第 8 章新增 18 个 `planned` 工作单元，并已补充 Applet / Manim / Diagnosis 全类型 scaffold 命令以减少后续手工建目录成本。
- 当前第 7 章 backlog 已无 `planned` item；第 8 章 backlog 应先按数字化必要性过滤，再按同一资源类型和同一推荐分支分批 scaffold，之后进入单资源精修。若继续深挖第 7 章，应优先推进数学审校、课堂节奏试读和资源状态升级，而不是继续新增同质资源。
