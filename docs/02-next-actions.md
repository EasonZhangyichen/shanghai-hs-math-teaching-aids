...
- [ ] 下一轮优先开 3 条质量修复/复核线：修复 `L05-A01` 平台 iframe 裁切与读数区高度；为 `L04-M01` 补齐视频 metadata 入口并清理平台 note 中的旧 `a cos theta` 表达；对 `L10-A01` 跑正式平台 iframe / 直达页浏览器复核并记录位置向量简写风险。
- [ ] 在上述三项完成前，不恢复第 8 章 3 生产 + 3 审核，也不把任何第 8 章资源提升到 `classroom_trial`、`release_candidate` 或 `published`。
