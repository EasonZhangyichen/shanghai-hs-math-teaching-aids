...
- [ ] 下一轮优先开 3 条小范围质量线：复核 `L05-A01` 真实触控屏拖拽手感和教师 12 分钟流程，修复或复核 `L10-A01` 平台 iframe 首屏适配，复核 `L04-M01` 平台视频播放、poster、暂停点和教师观看节奏。
- [ ] 在上述三项完成前，不恢复第 8 章 3 生产 + 3 审核，也不把任何第 8 章资源提升到 `classroom_trial`、`release_candidate` 或 `published`；`video_ready` 只表示平台视频入口可预览。
