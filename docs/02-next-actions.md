...
- [x] 对 `SH-HS-MATH-HJ-B2-C08-L04-M01` 开启单资源精修对话，补齐真实 Manim 场景、数学符号、动画节奏、预览渲染和审校记录；当前已导出 `mp4` / `webm` / poster，但顶层仍保持 `draft`，平台卡片仍保持 `metadata_ready`。
...
- [x] 总控合并生产线第三批后，确认 L05/L10 仍保持 `draft` / `self_checked_draft`，L04-M01 已有视频产物但平台卡片仍为 `metadata_ready`；下一步只进入数学审校、浏览器/视频复核和教师试读准备，不直接进入发布流程。
...
- [ ] 根据第 8 章 L05 / L10 / L04-M01 数学审核报告安排修复线：`L05-A01` 优先修正零向量投影边界、弱化 8.3 坐标读数误导；`L10-A01` 继续补实先修链、真实路径比较模型和教师脚本；`L04-M01` 清理 `theta` / `pi` / `a dot b` 等 fallback 并将 L04 收束到有向投影长度，不提前完成 L05 数量积定义。
- [ ] 根据第 8 章 L05 / L10 / L04-M01 浏览器与课堂报告安排修复线：`L05-A01` 优先处理首屏高度、平台 iframe 裁切、触控目标偏小和教师试读记录量；`L10-A01` 先按当前 runnable prototype 重新做浏览器/数学复核；`L04-M01` 以当前 `develop` 已有视频产物为准，单独做视频观看复核后再决定是否从 `metadata_ready` 升级。
- [ ] 第 8 章 L05 / L10 / L04-M01 修复完成前，暂停继续扩大第 8 章生产线；下一轮建议开 3 条质量修复线，而不是继续 3+3 新生产。
