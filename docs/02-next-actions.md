...
- [ ] 新开独立“数学公式显示修复线”，先审计 B2-C07 与 B2-C08 已实现 Applet 中课堂可见 `theta`、`pi/2`、横向纯文本分式和英文变量占位，按 `docs/content-standards.md` 的数学公式呈现标准修复；总控只分派和验收，不在总控线程直接修资源。
...
- [x] 完成第六轮第 8 章 L05 / L10 / L04-M01 补充审核闸门三三反馈的总控验收流程：逐条核对 `1097549`、`5260ba0`、`bda0e4b` 的分支、commit、docs-only 改动范围和验证结果后再集成；L05 仅进入教师限时试读准备，L10 仅进入 `math_review` 队列并保留平台 iframe hold，L04-M01 仅进入教师观看确认队列。
