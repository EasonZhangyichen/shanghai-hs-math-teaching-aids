...
- [ ] 根据审核线首批报告安排第 7 章小修复：清理 `L01-A01` README 中的 `theta` / `2pi` 旧口径，清理 `L06-A01` README 中的 `pi/2` / `kpi` / `2pi` ASCII 写法，并统一课程图谱中的 `omega`、`phi`、`pi/2 + kpi` fallback 表达。
- [ ] 根据浏览器大屏触控报告安排平台/资源优化：优先修复 `L03-A02` 平台 iframe 窄宽裁切与触控目标偏小；评估 `L05-A01` 是否压缩首屏高度或接受 iframe 内滚动风险；`L07-A01` 可考虑进入带风险记录的 `browser_review`。
- [ ] 根据课堂试读报告安排教学脚本优化：`L01-A01` 与 `L06-A01` 可交真实教师限时试读；`L05-A01` 需先拆分或压缩为 12 到 18 分钟课堂片段，暂不进入 `classroom_trial`。
