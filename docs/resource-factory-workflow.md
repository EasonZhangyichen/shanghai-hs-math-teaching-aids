...
当前覆盖必修第二册第 7 章三角函数和第 8 章平面向量：25 个资源工作单元，其中 15 个已实现，10 个待创建。第 8 章 planned item 已完成数字化必要性筛选，当前只保留必要性明确或已收窄的候选资源；8.3 暂缓资源等待教材或 dolearning 终核后再决定是否恢复。
...
当前必修第二册第 7 章已经没有 `planned` item；第 8 章“平面向量”已有 10 个 `planned` item，均来自数字化必要性筛选后的保留或收窄条目。下一轮 scaffold 仍应先按同一类型、同一推荐分支筛选第 8 章条目，并避开 8.3 暂缓资源。对已实现资源，下一步不是重复 scaffold，而是进入数学审校、课堂试读、浏览器复核和状态升级。
...
- 扩展线：围绕第 8 章筛选后保留的 10 个 planned item，先用 scaffold 小批量生成同类型资源骨架，随后按资源工厂节奏逐个精修。
