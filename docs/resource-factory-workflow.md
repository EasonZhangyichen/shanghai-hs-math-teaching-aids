...
当前覆盖必修第二册第 7 章三角函数、第 8 章平面向量，以及必修第一册第 5 章函数主线的少量 draft 候选：29 个资源工作单元，25 个已实现，4 个仍为 planned。第 8 章已完成数字化必要性筛选和骨架生成；8.3 暂缓资源等待教材或 dolearning 终核后再决定是否恢复。必修第一册第 5 章 4 个 planned item 仅来自来源核对后的数字化必要性初筛，未终核前不要直接 scaffold。
...
当前必修第二册第 7 章和第 8 章都已经没有 `planned` item；新增的 `planned` item 只来自必修第一册第 5 章函数主线 draft 图谱候选。下一轮 scaffold 应等待总控确认来源核对、课时边界和数字化必要性后再开始。对已实现资源，下一步不是重复 scaffold，而是进入单资源精修、数学审校、课堂试读、浏览器复核和状态升级。
