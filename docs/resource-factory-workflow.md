...
6. 如果发现跨资源的共性缺陷，例如公式显示、分式排版、希腊字母、iframe 首屏、触控热区或视频拖动问题，总控只负责拆分任务和验收规则；具体修复必须开独立工作对话，且每个对话只处理一个资源包或一组同类文件。
7. 完成后运行：
...
- 数学表达是否严谨。
- 数学公式显示是否符合教材书写习惯：希腊字母、分式、集合、区间、绝对值和函数式必须以数学排版呈现，不能把 `theta`、`pi/2`、横向纯文本分式暴露给课堂。
- 教学顺序是否适合课堂。
- 交互是否真的解决痛点。
- 诊断题是否能定位错因。
- 是否存在版权风险。
...
- 审核线：第 8 章 L01-L04 与 L05/L10/L04-M01 的两批审核报告都已合入总控，L05/L10/L04-M01 的首轮质量修复、修复后复核、平台入口闸门修复、小范围质量闸门复核和补充审核闸门报告也已合入。L05 触控热区与资源内布局自检通过，可进入带外层滚动说明的教师限时试读准备；L10 可进入 `math_review` 队列但仍保留 `hold_for_platform_iframe_fit`；L04-M01 保持 `video_ready` 预览入口并进入教师观看确认队列。下一步仍不应立刻扩大生产，而应继续处理 `L10-A01` 平台 iframe fit、`L05-A01` 真实触控/教师试读和 `L04-M01` 教师观看节奏；第 8 章教材目录未终核前，不恢复发布链路。
