...
9. 总控线程只做方向判断、质量闸门、任务拆分、分支核验、集成验证、合并推送和调度裁决。修复、资源生产、数学审核、浏览器审核和新功能实现必须开独立 `codex/*` 工作线程；总控线程不得直接生产资源、直接修资源包或在总控上下文中完成实现类工作。
10. 公式显示质量是内容质量的一部分。所有工作对话必须遵守 `docs/content-standards.md` 的“数学公式呈现标准”，用户可见区域不得出现 `theta`、`lambda`、`omega`、`kpi`、`pi/2`、横向纯文本 slash 分式等不符合数学书写习惯的表达。
...
- 发起修复、生产、审核和新功能任务时，只写清范围、禁止范围、验证命令和回报格式，然后交给独立 `codex/*` 工作线程执行。

建议分支：

- 默认停留在 `develop`。
- 总控对话不直接修资源包、平台功能、数学文本、浏览器问题或新功能实现。
- 如果要修改项目治理文档，应在独立 `codex/docs-*` 或 `codex/repair-*` 短分支完成后合回 `develop`。
- 一旦任务涉及资源源码、平台交互、新功能、数学语言修复、浏览器复核、审核结论或状态建议，必须拆成新工作对话。
...
如果你发现某类问题需要跨多个资源修复，例如数学公式显示、触控热区、iframe 首屏、视频播放或题组节奏，不要在总控对话中直接修。总控应先写清允许范围、禁止范围、验证命令和回报格式，再让独立 `codex/*` 工作对话处理单个资源或小批量同类资源；总控只负责回收分支、核验质量和集成推送。
