...
本资源对应沪教版高中数学必修第二册第 7 章 7.3“函数 `y = A sin(ωx + φ)` 的图像”。它服务于 `SH-HS-MATH-HJ-B2-C07-L05-A01` 参数变化 Applet 之前的概念拆解，帮助学生先看清变换顺序：
...
这个课时的困难不在“会不会拖滑块”，而在学生经常把多个参数同时看，导致把 `φ` 误当成实际平移量，或者把 `A` 的负号误解为横向变化。Manim 适合把这些操作拆成固定节奏的连续动画，让教师在关键处暂停追问，再切到 Applet 做验证。

## 课堂用法

- 概念导入：播放第一轮，先不让学生操作，只观察每个参数对应的方向。
- 关键暂停：在 `2x + π/2 = 2(x + π/4)` 和 `y = -1.5 sin(2(x + π/4))` 出现后追问“为什么不是左移 `π/2`”。
...
```bash
ffmpeg -y -ss 00:00:52 -i dist/final/SH-HS-MATH-HJ-B2-C07-L05-M01.mp4 -frames:v 1 -update 1 dist/final/SH-HS-MATH-HJ-B2-C07-L05-M01-poster.png
```
