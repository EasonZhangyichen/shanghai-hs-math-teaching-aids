# 审核记录

资源 ID：`SH-HS-MATH-HJ-B2-C08-L10-D01`

当前状态：`draft`

审核状态：`self_checked_draft`

## 自检记录

- 已对齐课时 `SH-HS-MATH-HJ-B2-C08-L10`「向量的应用」。
- 已生成 Diagnosis 必备文件。
- 已将空题组补为 6 题、10 分、建议 8 到 10 分钟完成的课堂即时诊断。
- 题组聚焦“第一步选择”，覆盖基底与路径、简洁坐标设点、数量积、投影分解和比例点表达。
- `metadata.status` 保持 `draft`，`compliance.review_status` 保持 `self_checked_draft`。
- 平台卡片继续保持 `metadata_ready`，以符合当前第 8 章草稿资源的平台测试口径；题组内容已补齐，后续由总控决定是否升级平台预览。
- metadata 的题型摘要暂保留平台草稿兼容标记，但真实题型与答案以 `item-bank.yaml` 的 6 题为准。

## 数学与教学边界

- 本诊断不处理复杂解析几何证明，不要求学生列直线方程或完成整题证明。
- 坐标法只作为“有天然直角边、边长或已给坐标时的简洁入口”出现，不作为 8.4 主线替代。
- 数量积只在垂直、夹角、长度比较和锐钝判断中触发，避免机械套公式。
- 投影与分解只用于方向分量、有效长度和垂直分解情境。

## 2026-05-19 质量线复核

- 数学试做通过：6 题均围绕 8.4“向量应用第一步”展开，使用基底与路径、比例点表达、简洁坐标设点、数量积入口和投影分解等已在第 8 章前序课时出现的工具。
- 课时边界通过：坐标法、数量积和投影只作为 8.4 综合应用的已学工具，不提前引入第 8 章以外的新方法，也不把题组扩展为复杂解析几何证明。
- 答案唯一性通过：Q01、Q03 为明确单选，Q02、Q04 的多选正确项和错误项区分稳定；Q05、Q06 的开放题均给出可评分的关键步骤与理由。
- 错因标签定位清晰：`coordinate_overreach`、`basis_path_gap`、`point_ratio_expression_gap`、`dot_product_trigger_gap`、`projection_decomposition_gap`、`coordinate_setup_gap` 分别对应“第一步选择”中的不同工具误判。
- 限时可行性判断：题组不要求完整证明，Q05、Q06 允许条目式回答时 8 到 10 分钟可完成；若要求写完证明过程会超出本诊断边界，教师说明已明确禁止这样使用。
- 状态边界：保持 `metadata.status: draft`、`compliance.review_status: self_checked_draft`、`platform_card.availability: metadata_ready`，本次不提升任何正式流转状态。

## 待复核

- 需数学教师确认 Q05 比例点表达的课堂语言是否与沪教版 8.4 例题口径一致。
- 需小组限时试做，确认 6 题能在 8 到 10 分钟内完成。
- 需后续诊断播放器接入后复核多选题、短解释题和错误分析题的标签统计口径。
- 第 8 章课时划分仍需纸质教材或已登录 dolearning 左侧目录终核；本包当前只保持 `draft` / `self_checked_draft` / `metadata_ready`。

## 2026-05-19 B2-C08 Diagnosis 审核线程复核

- 本轮只做题组质量审核，未新增题目，未改变 `metadata.status: draft`、`compliance.review_status: self_checked_draft` 或 `platform_card.availability: metadata_ready`。
- 数学口径复核：Q01 到 Q06 均服务 8.4“第一步选择”，坐标法、数量积和投影只作为已学工具入口，不扩展为完整解析几何证明。
- 已小修严格性表述：Q02 的夹角判断项补明“两个非零向量”，避免把数量积符号误用于零向量夹角边界。
- 限时节奏复核：题组只要求选择第一步和一句理由，Q05、Q06 条目式作答时 8 到 10 分钟可行；若教师要求完整证明则超出本诊断边界。
