...
建议放在「正切函数的性质」新授课后半段。学生先在 `SH-HS-MATH-HJ-B2-C07-L07-A01` 中观察分段图像、周期 `π` 对照、奇对称和值域归纳，再用本诊断任务检查是否能把性质写成完整、带条件的一般式。

## 6 到 8 分钟流程

1. 0 到 1 分钟：说明本题组重点不是背表格，而是每个结论都要带上定义域和分段条件。
2. 1 到 6 分钟：学生独立完成 6 题，Q04 和 Q06 接受短句或条目式订正，教师不提前提示“整体单调”的陷阱。
...
| `interval_formula_error` | 板书一个分支，再平移 `kπ`，注明 `k ∈ Z`。 | “端点为什么不能写成闭区间？” |
| `range_asymptote_conflict` | 只显示一个定义区间上的正切分支。 | “这一段的 y 值能不能覆盖任意给定的实数？” |
| `period_pi_confusion` | 打开 `x` 与 `x + π` 周期对照。 | “为什么正切的重复距离比正弦余弦短？” |
...
- 若 `range_asymptote_conflict` 高发，避免只说“值域为 R”，要让学生口头描述一个分支中的 y 值可以任意小、任意大，并覆盖任意给定实数。
- 若 `period_pi_confusion` 高发，要求学生用公式 `tan(x + π) = tan x` 和图像平移各解释一次。
