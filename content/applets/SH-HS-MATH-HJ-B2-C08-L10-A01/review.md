...
- 默认基向量：设 `p` 表示向量 `AB`，`q` 表示向量 `AC`。若 `AD/AB = m`、`AE/AC = n`，则 `AD = m p`、`AE = n q`，所以 `DE = AE - AD = -m p + n q`。
- 替换基向量：设 `p` 表示向量 `AB`，`r` 表示向量 `BC`。因为 `AC = p + r`，所以 `AE = n(p + r)`，`DE = AE - AD = (n - m)p + n r`。
- 位置向量简写：原 `D = mp`、`E = nq` 容易把点与向量直接等同；本轮改为 `AD = mp`、`AE = nq`。若课堂口头使用“D 点对应 mp、E 点对应 nq”，必须补明“以 A 为共同起点的位置向量”，并建议数学审校专项确认该简写是否进入正式脚本。
...
- 平台 iframe 复测绿灯：在 `864x560` 等效尺寸下，核心画板底部约 `329px`，比例滑块底部约 `383px`，路径按钮底部约 `155px`，路径等式底部约 `370px`，基向量切换底部约 `434px`，基向量等式底部约 `516px`，均落在 560px iframe 首屏内。
...
- `node --test content/applets/SH-HS-MATH-HJ-B2-C08-L10-A01/src/platform-fit.self-check.test.mjs`：通过，3 项资源级自检全部通过。
...
- 位置向量简写需数学审校专项确认：正式文件已改为 `AD = mp`、`AE = nq`，但课堂口头是否允许说“D 点对应 mp、E 点对应 nq”仍需审校者给统一口径。
...
- 建议：维持 `self_checked_draft`，专项浏览器修复可交总控复验后再解除 `hold_for_platform_iframe_fit`。
