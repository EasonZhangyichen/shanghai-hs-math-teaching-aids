...
本轮复核并修正 8.1.1 阶段的数学语言：平面网格中可以拖动蓝色 `AB` 定义向量，橙色 `CD` 始终按同一几何位移平移到新起点；读数区改为横向移动、纵向移动、长度、方向和起点对照。资源主线聚焦“相等向量可以平移”，不提前使用后续课时的解析化表示口径，不修改课程图谱，也不建议进入 `published`。

## 数学自检

- 非零向量相等的课堂口径：大小相等且方向相同。
- 网格读数口径：用“从起点到终点横向移动几格、纵向移动几格”作为几何证据，不采用后续解析化记法。
- 起点变化对照：拖动 `C`、`D` 或橙色向量整体时，只改变 `C` 和 `D` 的位置，不改变横向移动、纵向移动、长度和方向。
- 零向量边界：当 `A` 与 `B` 重合时显示长度为 0，方向不规定。
- 资源边界：不引入向量加减法、数乘、后续解析化运算训练或 8.3 课时划分判断。

## 本轮修改

- `src/index.html`：将界面读数、归纳语和辅助线标签改为横向/纵向移动、长度和方向；移除后续解析化展示。
- `metadata.yaml`：将学习目标、数学边界、交互说明、反馈证据收回到几何位移、大小和方向语言。
- `README.md`：同步清理后续解析化和角度化前置口径。
- `teacher-script.md`：将板书和追问改为“移动方式、大小、方向”。
- `student-task.md`：将记录任务改为横向/纵向移动观察和相等向量判断。
...
- 尚未经过数学审校者复核，尤其需要确认 8.1.1 阶段保留横向/纵向移动读数是否适合班级基础。
