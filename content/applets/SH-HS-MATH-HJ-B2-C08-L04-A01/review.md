...
建议保持 `self_checked_draft`。本轮只修复课堂显示、8.3 坐标前置风险和 560px 高度 iframe 紧凑显示，不提升到浏览器审核、课堂试用、发布候选或发布状态。
...
当前 `src/index.html` 可直接打开运行，metadata 表示平台可生成 iframe 预览。该修正只表示“可运行预览入口”与真实文件状态一致，不代表数学审校、浏览器审核、课堂试用或发布通过。
