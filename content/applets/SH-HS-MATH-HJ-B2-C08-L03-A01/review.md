...
本轮已将 scaffold 占位入口推进为可运行 Applet 自检草稿：学生可以拖动蓝色原向量 `a`，并通过 λ 滑块连续观察橙色 `λa` 的同向伸缩、零向量边界和反向伸缩。资源主线聚焦“λ 的符号决定方向，|λ| 决定长度倍率”，不处理 8.3 暂缓资源，不修改课程图谱；因第 8 章教材来源和课时边界未终核，本资源只保持 `draft` / `self_checked_draft` / `metadata_ready` 草稿边界。
...
- 尚未经过独立数学教师复核，尤其需要纸质教材和已登录平台终核确认 L03 与 8.3 的课时边界是否已足够清晰。
