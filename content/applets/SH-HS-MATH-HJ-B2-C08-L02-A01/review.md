...
- 加法：首尾相接模式中，`b` 被平移到 `a` 的终点，结果向量从共同起点指向最终终点，表示向量和 `a + b`。
- 加法：平行四边形模式中，`a`、`b` 共起点，平移副本补出第四个顶点，对角线表示同一个向量和 `a + b`。
- 减法：减法模式中，向量差 `a - b` 被解释为 `a + (-b)`，并用从 `B` 指向 `A` 的向量检查端点顺序。
- 证据口径：当前状态只作为拖拽构造的几何证据，使用水平位移、竖直位移、首尾相接、路径和端点顺序，不扩展到 8.3 后续表示方式的系统教学。

## 本轮补充修正

- 清理界面、README、教师脚本、学生活动和 metadata 中提前使用的后续解析化口径。
- 将 `a`、`b`、向量和 `a + b`、向量差 `a - b` 的证据改成网格中的几何位移和路径证据。
- 微调平台 iframe 首屏：收紧外边距和画板最小高度，让控制区在首屏内可扫读；按钮高度与画布端点命中半径按触控操作放大。
- 保持资源顶层 `draft` 与 `self_checked_draft`，不提升审核或发布状态。

## 2026-05-20 语言与符号修复

- 将课堂可见的 ASCII 路径箭头统一改为 `O → A → S`。
- 强化首尾相接路径、总位移、向量和 `a + b` 与向量差 `a - b` 的课堂表达。
- 补清 `-b` 为与 `b` 等长反向的相反向量，强调 `a - b = a + (-b)` 与从 `B` 指向 `A` 的端点顺序。
- 将 README 和 metadata 中的证据口径收束为几何位移描述，明确不引入坐标分量运算。
- 本轮未修改 `metadata.status` 或 `compliance.review_status`。
...
- `npm run verify` 会再生 `apps/web/src/data/workspace-data.json` diff；本分支不提交该 generated file，等待总控统一再生。
